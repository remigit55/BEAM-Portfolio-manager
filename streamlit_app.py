...
def load_or_reload_portfolio(source_type, uploaded_file=None, google_sheets_url=None):
    ...
    if df_loaded is not None and not df_loaded.empty:
        ...
        required_columns = ['Quantité', 'Acquisition', 'Objectif_LT', 'Catégorie', 'Devise', 'H52'] # Ajout de H52
        ...
        if missing_columns:
            ...
            for col in missing_columns:
                if col in ['Quantité', 'Acquisition', 'Objectif_LT', 'H52']: # Ajout de H52
                    df_loaded[col] = 0.0
                elif col == 'Catégorie':
        ...
        try:
            ...
            df_loaded['H52'] = pd.to_numeric(df_loaded['H52'], errors='coerce').fillna(0) # Conversion de H52
...
if 'df' in st.session_state and isinstance(st.session_state.df, pd.DataFrame) and not st.session_state.df.empty and 'Ticker' in st.session_state.df.columns:
    # Use actual calculated columns for totals if they exist
    total_valeur = st.session_state.df['Acquisition (Devise Cible)'].sum() if 'Acquisition (Devise Cible)' in st.session_state.df.columns else 0.0
    total_actuelle = st.session_state.df['Valeur Actuelle'].sum() if 'Valeur Actuelle' in st.session_state.df.columns else 0.0
    total_h52 = st.session_state.df['H52'].sum() if 'H52' in st.session_state.df.columns else 0.0
    total_lt = st.session_state.df['Objectif_LT'].sum() if 'Objectif_LT' in st.session_state.df.columns else 0.0
else:
    total_valeur = 0.0
    total_actuelle = 0.0
    total_h52 = 0.0
