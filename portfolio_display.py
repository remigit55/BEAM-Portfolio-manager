...
def afficher_synthese_globale(total_valeur, total_actuelle, total_h52, total_lt):
    ...
    if "df" in st.session_state and st.session_state.df is not None and not st.session_state.df.empty:
        ...
        # Cette valeur servira de référence pour les objectifs de toutes les autres Categories
        ...
        if target_minieres_pct > 0: # Évite la division par zéro
            ...
            # on ne peut pas baser le total théorique sur cette Categorie.
            # On revient à la valeur totale actuelle du portefeuille comme base.
            # st.warning("L'objectif pour 'Minières' est de 0% ou 'Minières' n'a pas de valeur. Le calcul de l'objectif par Categorie se basera sur la valeur totale actuelle du portefeuille.")
        ...
        if pd.isna(theoretical_portfolio_total_from_minieres) or np.isinf(theoretical_portfolio_total_from_minieres) or theoretical_portfolio_total_from_minieres <= 0:
            ...
            # st.warning("Le calcul de la valeur totale théorique du portefeuille basée sur les Minières a échoué ou est nulle. Le calcul de l'objectif par Categorie se basera sur la valeur totale actuelle du portefeuille.")
