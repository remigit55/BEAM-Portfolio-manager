...
def convertir(val, source_devise, devise_cible, fx_rates_or_scalar, fx_adjustment_factor=1.0):
    ...
    Peut accepter un dictionnaire de taux de change ou un taux scalaire direct.
...
def afficher_portefeuille():
    ...
    # --- GESTION DES PENCE BRITANNIQUES (GBp) ---
    if "Devise" in df.columns:
        ...
        df.drop(columns=['original_devise_lower_for_gbp_check'], inplace=True)
    else:
        st.warning("Colonne 'Devise' absente. Aucune conversion pence-vers-livre appliquée.")
    ...
    df[['Valeur_conv', 'Taux_FX_Acquisition']] = df.apply(
        lambda x: convertir(x["Valeur Acquisition"], x["Devise"] if not is_pence_denominated(x["Devise"]) else "GBP", devise_cible, fx_rates, x["Facteur_Ajustement_FX"]), 
        axis=1, result_type='expand'
    )
    df[['Valeur_Actuelle_conv', 'Taux_FX_Actuel']] = df.apply(
        lambda x: convertir(x["Valeur_Actuelle"], x["Devise"] if not is_pence_denominated(x["Devise"]) else "GBP", devise_cible, fx_rates, x["Facteur_Ajustement_FX"]), 
        axis=1, result_type='expand'
    )
    df[['Valeur_H52_conv', 'Taux_FX_H52']] = df.apply(
        lambda x: convertir(x["Valeur_H52"], x["Devise"] if not is_pence_denominated(x["Devise"]) else "GBP", devise_cible, fx_rates, x["Facteur_Ajustement_FX"]), 
        axis=1, result_type='expand'
    )
    df[['Valeur_LT_conv', 'Taux_FX_LT']] = df.apply(
        lambda x: convertir(x["Valeur_LT"], x["Devise"] if not is_pence_denominated(x["Devise"]) else "GBP", devise_cible, fx_rates, x["Facteur_Ajustement_FX"]), 
        axis=1, result_type='expand'
    )
    ...
    try:
        valeur_acquisition_source_idx = list(df_disp.columns).index("Valeur Acquisition (Source)") + 1
        st.markdown(f"""
            <style>
...
def afficher_synthese_globale(total_valeur, total_actuelle, total_h52, total_lt):
    """
    Affiche la synthèse globale du portefeuille, y compris les métriques clés et le tableau de répartition par Catégories avec les objectifs.
    """
    devise_cible = st.session_state.get("devise_cible", "EUR")
