...
def afficher_portefeuille():
    ...
    if ticker_col and not df[ticker_col].dropna().empty:
        ...
        # Removed: df["Last Price"] = df[ticker_col].map(lambda t: st.session_state.momentum_results_cache.get(t, {}).get("Last Price", np.nan)) # Removed to exclude "Dernier Prix"
        ...
        # Removed: df["Last Price"] = np.nan # Removed to exclude "Dernier Prix"
        df["Momentum (%)"] = np.nan
        df["Z-Score"] = np.nan
        df["Signal"] = ""
        df["Action"] = ""
    ...
    cols = [
        ...
        # Removed: "Last Price", # Removed to exclude "Dernier Prix"
    ...
    labels = [
        ...
        # Removed: "Dernier Prix", # Removed to exclude "Dernier Prix"
        "Momentum (%)", "Z-Score",
        "Signal", "Action", "Justification"
    ]
    ...
    <style>
        ...
        .portfolio-table {{
            min-width: 2400px;  # Reduced from 2500px due to removal of "Dernier Prix"
