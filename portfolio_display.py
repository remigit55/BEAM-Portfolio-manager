...
def afficher_portefeuille():
    ...
    if "Categories" in df.columns:
        ...
        st.warning("ATTENTION: La colonne 'Categories' est introuvable. La colonne 'Catégorie' sera 'Non Classé'.")
    ...
    df['Gain/Perte (%)'] = np.where(
        (df['Valeur_conv'] != 0) & (pd.notna(df['Valeur_conv'])),
    ...
    for col_name, dec_places in [
        ...
        if col_name in df.columns:
            ...
            elif col_name in ["Gain/Perte (%)", "Momentum (%)", "Z-Score"]:
    ...
    <style>
        ...
        .category-summary-table .text-align-left {{ text-align: left !important; }}
        .category-summary-table .text-align-right {{ text-align: right !important; }}
    </style>
    ...
    for lbl in df_disp.columns:
        sort_icon = " ▲" if st.session_state.sort_column == lbl and st.session_state.sort_direction == "asc" else " ▼" if st.session_state.sort_column == lbl else ""
...
def afficher_synthese_globale(total_valeur, total_actuelle, total_h52, total_lt):
    """
    Affiche la synthèse globale du portefeuille, y compris les métriques clés et le tableau de répartition par catégorie.
    """
    devise_cible = st.session_state.get("devise_cible", "EUR")
    ...
    if "df" in st.session_state and st.session_state.df is not None and not st.session_state.df.empty:
        ...
        if 'Catégorie' not in df.columns:
            st.error("ERREUR: La colonne 'Catégorie' est manquante. Vérifiez la colonne 'Categories' dans votre fichier.")
            st.info(f"Colonnes disponibles: {df.columns.tolist()}")
            return

        portfolio_total_value = total_actuelle
        if portfolio_total_value == 0:
            st.info("La valeur totale actuelle est 0. Impossible de calculer la répartition.")
        ...
        category_values = df.groupby('Catégorie')['Valeur_Actuelle_conv'].sum(skipna=True)
        ...
        ideal_portfolio_total = valeur_totale_reference / pourcentage_cible_reference if pourcentage_cible_reference > 0 else 0.0

        if pourcentage_cible_reference == 0 and valeur_totale_reference > 0:
            st.warning(f"La catégorie '{REFERENCE_CATEGORY}' a une valeur positive ({format_fr(valeur_totale_reference, 2)} {devise_cible}) mais un objectif de 0%. L'ajustement ne peut pas être calculé.")
        ...
        for category in all_relevant_categories:
            ...
            valeur_pour_atteindre_objectif_str = f"{format_fr(value_to_adjust, 2)} {devise_cible}" if pd.notna(value_to_adjust) else "N/A"
        ...
        """
        components.html(summary_html_code, height=df_allocation.shape[0] * 35 + 100)
    else:
        st.info("Le DataFrame de votre portefeuille n'est pas disponible ou ne contient pas la colonne 'Catégorie'.")
        st.warning("Veuillez importer votre portefeuille et vérifier la présence de la colonne 'Categories'.")
