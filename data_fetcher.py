...
# Liste complète des tickers (format yfinance corrigé pour certains marchés)
tickers = [
...
for ticker in tickers:
    try:
        ...
        plt.plot(df.index, df['Z_Momentum'], label=f'{ticker} - Z Momentum')
        plt.axhline(0, color='gray', linestyle='--')
        plt.axhline(2, color='red', linestyle='--')
        plt.axhline(-2, color='green', linestyle='--')
        plt.title(f'Oscillateur de Momentum - {ticker}')
        plt.legend()
        plt.grid(True)
        plt.show()
